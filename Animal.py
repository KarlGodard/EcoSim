...
class Animal:
    ...
    def waterReact(self, surroundings):
        ...
        if nearbyWater == None:
            ...
            if canDrink is True:
                ...
                self.water += (0.25 * self.maxWater)
                self.position_x = nearbyWater[0]
                self.position_y = nearbyWater[-1]
    ...
    def checkIsFertile(self):
        if self.food < (self.maxFood * 0.75) or self.water < (self.maxWater * 0.75): #changed from 0.5
            self.isFertile = 0
        else:
            self.isFertile = 1
            # do age later
...
class Predator(Animal):
    def __init__ (self, xmax, ymax, position_x=None, position_y=None, animalID=None):
    ...
    def predReact(self, animal_sr):
        ...
        invalid_locs = animal_sr.getNearbyPrey() + animal_sr.getNearbyPredators()
